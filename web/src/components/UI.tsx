...
import { ArrowUp, ArrowDown, AlertTriangle, Diamond, Circle } from 'lucide-react';
...
export function StatCard({ label, value, sub, trend, color = 'default' }: { label: string; value: string; sub?: ReactNode; trend?: number; color?: 'green' | 'red' | 'gold' | 'default' }) {
  ...
  return (
    <div className="glass rounded-xl p-4">
      ...
      {trend !== undefined && <p className={clsx('text-xs mt-1 font-medium inline-flex items-center gap-0.5', trend >= 0 ? 'text-emerald-400' : 'text-red-400')}>{trend >= 0 ? <ArrowUp size={12} /> : <ArrowDown size={12} />} {Math.abs(trend)}%</p>}
    </div>
  );
}
...
export function FlagBadge({ label, severity }: { label: string; severity: 'High' | 'Medium' | 'Low' }) {
  ...
  const Icon = severity === 'High' ? AlertTriangle : severity === 'Medium' ? Diamond : Circle;
  return <span className={clsx('inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-lg border', colors[severity])}><Icon size={12} className={severity === 'Low' ? 'fill-current' : ''} /> {label}</span>;
}
